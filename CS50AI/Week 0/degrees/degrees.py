...
def main():
    ...
    # source = person_id_for_name("Leonardo DiCaprio")
    if source is None:
        sys.exit("Person not found.")
    target = person_id_for_name(input("Name: "))
    # target = person_id_for_name("Leonardo DiCaprio")
    if target is None:
        sys.exit("Person not found.")
...
def shortest_path(source, target):
    ...
    while True:
        ...
        # Add the node to explored nodes
        ...
        for movie_id, person_id in neighbors:
            if person_id not in explored_nodes:
                if not frontier.contains_state(person_id):
                    neighbour_node = Node(person_id, node, movie_id)
